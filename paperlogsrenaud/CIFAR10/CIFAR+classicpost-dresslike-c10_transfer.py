...
path =  "paperlogs/CIFAR10/CC_classicpost-c10_transfer/"+date+"/"
os.makedirs(path)
...
class CircuitCentric(nn.Module):
    def __init__(self, qubits=7, n_layer=20):
        ...
        self.intermediate = nn.Linear(2**qubits,2**qubits) 
        self.final = nn.Linear(2**qubits, 10) # nombre de classes, todo as a variable
    ...
    def forward(self, x):
        ...
        output = self.intermediate(torch.real(output))
