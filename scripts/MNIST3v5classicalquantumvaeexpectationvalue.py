...
path = create_run_path("paperlogs/3v5/ClassicalQuantumVaeExpectationValue")
...
class MLP(nn.Module):
    def __init__(self, qubits=10):
        ...
        self.expvalcircuit = qml.qnn.TorchLayer(self.construct_circuit(), weight_shapes={}).to(device)
    def forward(self, x):
        x = self.expvalcircuit(x)
        return self.model(torch.real(x))
