...
class NeuralNetwork(nn.Module):
    def __init__(self, wires=8 , embedings = 1):
        ...
        #self.wrealmu = torch.Tensor(1000,2 ** len(self.wires))
        #self.wrealsigma = torch.Tensor(1000, 2 ** len(self.wires))
        #self.wimgmu = torch.Tensor(1000, 2 ** len(self.wires))
        #self.wimgsigma = torch.Tensor(1000, 2 ** len(self.wires))

        self.prefilter = nn.Sequential(
            nn.Linear(1000, 2 ** len(self.wires)),
            nn.Tanh()
        )

        self.realmu = nn.Sequential(
            nn.Linear(2 ** len(self.wires), 2 ** len(self.wires)),
        ...
        self.decoder = nn.Sequential(
            nn.Linear(2**len(self.wires), 65536),
            nn.BatchNorm1d(65536),
            nn.ReLU(),
            nn.Unflatten(1, (1024, 8, 8)),
            nn.ConvTranspose2d(in_channels=1024, out_channels=512, kernel_size=3, padding=1, stride=1),
            nn.BatchNorm2d(num_features=512),
            nn.ReLU(),
            nn.ConvTranspose2d(in_channels=512, out_channels=256, kernel_size=3, padding=1, stride=2, output_padding=1),
            nn.BatchNorm2d(num_features=256),
            nn.ReLU(),
            nn.ConvTranspose2d(in_channels=256, out_channels=128, kernel_size=3, padding=1, stride=1),
            ...
            nn.ReLU(),
            nn.ConvTranspose2d(in_channels=32, out_channels=32, kernel_size=3, padding=1, stride=1),
            nn.ReLU(),
            nn.ConvTranspose2d(in_channels=32, out_channels=32, kernel_size=3, padding=1, stride=1),
            nn.ReLU(),
    ...
    def forward(self, x):
        ...
        return decode.view(-1, 3, 32, 32) , self.encodedstate
    def get_latent(self, x):
        #x = self.flatten(x)
        encodetemp = self.encoder(x)
        encodetemp = self.prefilter(encodetemp)
...
#optimizerD = torch.optim.Adam(model.parameters(), lr=0.00002)#must have low learning rate 0.0002
def getnoise(inputs):
    ...
    for batch, (X, y) in enumerate(dataloader):
       ...
       #meanreal = torch.mean(realoutput)
        ...
        if discriminatoronly == False:
            ...
            loss = nllloss + 0.000001*model.kl #+ 0.000001 *c_weight* cycleloss
