...
path = create_run_path("log/hugging face vae/vae kl", timestamp=date)
...
def train(dataloader, model, loss_fn, optimizer, noise=False, epoch=0):
    ...
    for batch, (X, y) in enumerate(dataloader):
        ...
        pred = output.sample
        loss = loss_fn(pred, X) + 0.000001*kl
...
def test(dataloader, model, loss_fn,noise=False,epoch=0):
    ...
    with torch.no_grad():
        for X, y in dataloader:
            ...
            out, kl = model(inputs)
            pred = out.sample
            test_loss += loss_fn(pred, X).item() + 0.000001*kl
...
if __name__ == '__main__':
    ...
    model.load_state_dict(torch.load(registered_model_path("hf_mnist_autoencoderkl")))
