...
class NeuralNetwork(nn.Module):
    def __init__(self, wires=8):
        ...
        self.realmu = nn.Sequential(
            nn.Linear(512, 2 ** len(self.wires)),
        ...
        self.decoder = nn.Sequential(
            ...
            nn.ConvTranspose2d(in_channels=1, out_channels=32, kernel_size=3, padding=0, stride=1, dilation=2),
            nn.BatchNorm2d(num_features=32),
            nn.ReLU(),
            nn.ConvTranspose2d(in_channels=32, out_channels=16, kernel_size=3, padding=0, stride=1,dilation=2),
            nn.BatchNorm2d(num_features=16),
            nn.ReLU(),
            nn.ConvTranspose2d(in_channels=16, out_channels=8, kernel_size=3, padding=0, stride=1,dilation=2),
            nn.BatchNorm2d(num_features=8),
            nn.ReLU(),
            nn.ConvTranspose2d(in_channels=8, out_channels=1, kernel_size=3,padding=1, stride=1),
            nn.Tanh(), #nn.sigmoid
        )
...
optimizerD = torch.optim.Adam(modelDCNN.parameters(), lr=0.0002)#must have low learning rate 0.0002
def getnoise(inputs):
    ...
    for batch, (X, y) in enumerate(dataloader):
       ...
       #meanreal = torch.mean(realoutput)
        ...
        if discriminatoronly == False:
            ...
            loss = nllloss + 0.00001*model.kl + 0.000001 *c_weight* cycleloss
