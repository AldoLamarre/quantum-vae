...
path = create_run_path("paperlogs/CIFAR10/CC_classicpost")
...
batch_size = 128
classicpost_bundle = build_classic_post_data_bundle(training_data, test_data, batch_size=batch_size)
train_set = classicpost_bundle["train_set"]
val_set = classicpost_bundle["val_set"]
train_dataloader = classicpost_bundle["train_dataloader"]
val_dataloader = classicpost_bundle["val_dataloader"]
test_dataloader = classicpost_bundle["test_dataloader"]
...
class CircuitCentric(nn.Module):
    def __init__(self, qubits=5, n_layer=20):
        ...
        self.final = nn.Linear(qubits, 10)
    ...
    def forward(self, x):
        ...
        output = self.final(torch.real(output))
        return output
...
def train(dataloader, model,model_vae,loss_fn, optimizer, epoch=0):
    ...
    for batch, (X, y) in enumerate(dataloader):
        ...
        loss = loss_fn(torch.abs(out), torch.nn.functional.one_hot(y.to(device)).to(torch.float32))
...
def test(dataloader, model,model_vae,loss_fn):
    ...
    with torch.no_grad():
        for X, y in dataloader:
            ...
            test_loss += loss_fn(pred,  torch.nn.functional.one_hot(y.to(device)).to(torch.float32)).item()
