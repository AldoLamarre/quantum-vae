...
class NeuralNetwork(nn.Module):
    def __init__(self, wires=8):
        ...
        self.dev = qml.device("default.qubit", wires = self.wires, shots=1000)
    ...
    def contruct_circuit(self):
        ...
        @qml.qnode(self.dev, interface="torch", wires=self.wires, shots=1000)
        def circuit(inputs):
            ...
            #for i in self.wires:
                #qml.BitFlip(0.21, i)
            return qml.classical_shadow(wires=self.wires)
        return circuit
...
def train(dataloader, model, loss_fn, optimizer, noise=False):
    ...
    for batch, (X, y) in enumerate(dataloader):
        ...
        loss = loss_fn(pred, X) + 0.000001*model.kl + 0.0000001*cycleloss
...
if __name__ == '__main__':
    ...
    torch.save(model.state_dict(), "shadowvariationalautoencodertestpennylane five.pt")
