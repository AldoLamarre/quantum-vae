...
class NeuralNetwork(nn.Module):
    ...
    def contruct_circuit_example(self):
        # @qml.batch_input(argnum=1)
        @qml.qnode(self.dev, diff_method="backprop", interface="torch",wires=self.wires)
        def circuit(inputs):
            qml.QubitStateVector(torch.sqrt(inputs[:, 0:4]), wires=self.wires)
            unitary = qml.ArbitraryUnitary(weights=inputs[:, 4:20], wires=self.wires)
            qml.apply(unitary)
...
def train(dataloader, model, loss_fn, optimizer, noise=False):
    ...
    for batch, (X, y) in enumerate(dataloader):
        ...
        loss = nllloss + 0.00001*model.kl + 0.0000001 * cycleloss
