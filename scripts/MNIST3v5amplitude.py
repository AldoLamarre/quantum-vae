...
class CircuitCentric(nn.Module):
    def __init__(self, qubits=5, n_layer=10):
        super(CircuitCentric, self).__init__()
        self.dev = qml.device('default.qubit', wires=qubits*2)
...
def test(dataloader, model,model_vae,loss_fn):
    ...
    with torch.no_grad():
        for X, y in dataloader:
            ...
            pred = model(X) #the complex value is zero
...
if __name__ == '__main__':
    ...
    for t in range(epochs):
        ...
        loss = test(val_dataloader, model,modelVAE,loss_fn)