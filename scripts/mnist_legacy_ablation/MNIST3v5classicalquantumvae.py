...
path = create_run_path("paperlogs/3v5/ClassicalQuantumVae")
...
def train(dataloader, model,model_vae,loss_fn, optimizer, epoch=0):
    ...
    for batch, (X, y) in enumerate(dataloader):
        ...
        out = model(torch.cat(((state.real),(state.imag)), dim=1))
...
def test(dataloader, model,model_vae,loss_fn):
    ...
    with torch.no_grad():
        for X, y in dataloader:
            ...
            pred = model(torch.cat((torch.real(state), torch.imag(state)), dim=1))
...
if __name__ == '__main__':
    ...
    for t in range(epochs):
        ...
        if val_loss < best_val_loss:
            ...
            print(f"New best validation loss: {best_val_loss:.4f}, accuracy: {best_val_loss_accuracy:.4f} - Model saved.")
